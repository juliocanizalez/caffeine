...
use crate::domain::ports::{PowerGuard, PowerManager};
...
impl AssertionGuard {
    fn acquire(prevent_display: bool) -> Result<Self, String> {
...
// SAFETY: AssertionGuard holds Option<u32> (IOPMAssertionID), which is Send.
unsafe impl Send for AssertionGuard {}

impl PowerGuard for AssertionGuard {}

// ── Public infrastructure type ────────────────────────────────────────────────

pub struct IokitPowerManager;

impl PowerManager for IokitPowerManager {
    fn acquire(&self, prevent_display: bool) -> Result<Box<dyn PowerGuard>, String> {
        AssertionGuard::acquire(prevent_display).map(|g| Box::new(g) as Box<dyn PowerGuard>)
    }
}
